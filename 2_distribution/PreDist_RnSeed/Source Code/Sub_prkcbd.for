...
C DEBUGGING:      
C      WRITE (31, '(/A)') ' After intocc processing'
C
C      DO CC = 1,3
C         WRITE (31, '(A,5I3,4F10.3,F10.1)') 'L,INTOCC,J3,J6,J7,RAN5,
C     ACAPK,FREEPRK,HC,INCOME',L(CC),INTOCC(CC),J3(CC),J6(CC),J7(CC),
C     BRAN5(CC),CAPK(CC),FREEPRK(CC),HC(CC),INCOME(CC)
C      ENDDO   
...
C --
	IF (TRACE) THEN
     ...
     A prkadj(3,2)
	    WRITE (31, '(A,2F10.2)') '   FreeParking=', prkadj(4,1), 
     A prkadj(4,2)
	    WRITE (31, '(A,2F10.2)') '   FreeParking=', prkadj(5,1), 
     A prkadj(5,2)        
