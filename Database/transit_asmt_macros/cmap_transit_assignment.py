...
class TransitAssignment(_m.Tool()): #, gen_utils.Snapshot
    ...
    assignment_only = True  ## -- Heither, test this implementation
    ...
    def __init__(self):
        ...
        self.vots = {"uc1": 8.63, "uc2": 27.00, "uc3": 80.00}  # by user class - 2.30 $/hr*2.25 (8.63 cents/min), 7.20 $/hr*2.25 (27.00 cents/min), 24.00 $/hr*2 (80.00 cents/min), Heither: TBM VOT rates * perception factors
        ...
        self.basematrixnumber = 0 ##-- Heither, was 500, this will start skim matrix numbers at 1100
    ...
    def __call__(self, period, matrix_count, scenario, assignment_only=False, skims_only=False, summary=True,
        ...
        with self.setup(attrs):
            ...
            if not assignment_only:
                ...
                for amode in ["WALK"]:      ##-- Heither, just WALK for TBM
    ...
    def generate_matrix_list(self, period, scenario):
        ...
        self._count = {"ms": 2, "md": 100, "mo": 100, "mf": 1999} ##-- Heither: "mf" was 8500 + 16*3*5, change to 1999
    ...
    def transit_skim_matrices(self, period, scenario, num_processors):
        ...
        ##for amode in ["WALK", "PNROUT", "PNRIN", "KNROUT", "KNRIN"]:
        for amode in ["WALK"]:     ##-- Heither, only create Walk matrices for TBM   
            for uc in self.user_classes:
                for mname in self.skim_matrices:
                    ##-- Heither, 10-28-22: renumber so matrices start at 1100
                    temp = self.add_matrices(int(period) * 100 + 1000 + idx, "%s_%s_%s__%s"%(mname, amode, self.user_class_labels[int(uc)], self.periodLabel[int(period)]), scenario)
                    idx += 1                    
        return(True)
    ...
    def run_assignment(self, period, network, skims_only, num_processors, use_ccr):
        ...
        walk_modes = ["u", "v", "w", "x", "y", "z", "m", "c", "b", "r", "t", "d", "k"]     ## -- Heither: added v|w|y|z|k
        ...
        if skims_only:
            ...
            access_modes = ["WALK"]         ## -- Heither: Unlike the ABM, the TBM is doing it all within the WALK access mode (convolution of trips has occurred)
            skim_parameters = OrderedDict([
                ("WALK", {
                    "modes": walk_modes + local_bus_mode + premium_modes,
                    "journey_levels": []
