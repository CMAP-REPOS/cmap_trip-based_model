...
# (keep all zones from above that are NOT "illinois balance," "indiana," or "wisconsin")
...
def get_matrices(mtx_list):
    ...
    #error catching mtx_list format and contents
    if any(len(lst)!=2 for lst in mtx_list): #if the format of the list is wrong
    ...
    if any(lst[1][:2] != 'mf' for lst in mtx_list): #if list of mtces aren't all 'mf'
        raise ValueError('get_matrices() takes a list of sublists, where each sublist contains the name for the matrix and the id: e.g., ["bplate", "mf4"]')
    if any(emmebank.matrix(lst[1]) is None for lst in mtx_list): #if mtx doesn't exist, send error
...
# function for calculating weighted averages of pandas dataframes, given a desired output column name, a value, a weight, and a groupby column
...
trips = pd.merge(pq_df, eda_share_df, on='o_zone', how='left')
trips.eval('eda_trips = eda_share * trips', inplace=True)
...
trips['gen_purpose'] = trips['purpose'].map(gen_purpose).astype('category')
...
trips['gen_mode'] = trips['mode'].map(gen_mode).astype('category')

#error catch purpose
if len(trips.loc[trips['gen_purpose'].isnull()])>0:
    raise ValueError(f"{len(trips.loc[trips['gen_purpose'].isnull()])} trips exist in parquet that were not given HW, HO, or NH purpose. What purposes are those? Re-check coding.")
#error catch mode
if len(trips.loc[trips['gen_mode'].isnull()])>0:
    raise ValueError(f"{len(trips.loc[trips['gen_mode']].isnull())} trips exist in parquet that have modes not 1-9. What modes are those? Re-check coding.")

# flag 7-county region trips, and filter
o_in_7cty_bool = trips['o_zone'].astype(int).isin(zones_7county) #boolean id-ing origin in 7-county region
d_in_7cty_bool = trips['d_zone'].astype(int).isin(zones_7county) #boolean id-ing destination in 7-county region

trips = trips.loc[o_in_7cty_bool | d_in_7cty_bool].copy()
...
## --
## CALCULATE AM AND PM PEAK TRAVEL TIME & DISTANCE, HWY
## --
...
## -- 
## SORT AM AND PM PEAK TRAVEL TIME & DISTANCE (HWY)
...
time_bin_auto = auto_trips_tod37_hbw.groupby(['timeperiod','time_bin']).agg({'trips':'sum','eda_trips':'sum'})
...
daily_truck = daily_truck.agg({'tot_truck':'sum', 'eda_truck':'sum'})
...
time_bin_transit = transit.groupby(['timeperiod','time_bin']).agg({'hbw_trips':'sum','eda_hbw_trips':'sum'})
...
with open(rsp_out_csv, 'w') as file:
    ...
    for category in rsp_dict.keys():
        ...
        if type(rsp_dict[category])==dict:
            ...
            writer.writerows(rsp_dict[category])
